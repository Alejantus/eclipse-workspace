...
public class GestionHabitaciones {
	...
	public void mostrarFactura(int numH) {
		...
		for (int i = 0; i < lista.length; i++) {
			System.out.printf("%.2f\t%B\t%s\t%d\t%d", lista[numH - 1].getPrecioBase(), lista[numH - 1].isOcupado(),
					lista[numH - 1].getNombreCliente(), lista[numH - 1].getDiasOcupacion(),
					lista[numH - 1].getNumOcupantes());
		}
	}
	...
	public void mostrarLista(int numH) {
		for (int i = 0; i < numH; i++) {
			System.out.println((i+1)+" "+lista[i]);
			if (lista[i] instanceof Suite) {
				((Suite)lista[i]).mostrarDetalle();
